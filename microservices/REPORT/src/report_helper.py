...
import textwrap
from llm_providers.llm_provider import LLMProvider, get_provider
from dotenv import load_dotenv

load_dotenv()

TEMPLATE = """
Du bist Arzt in einem medizinischen Diagnostiklabor und erstellst einen detaillierten medizinischen Diagnostikbericht für einen Patienten.
Der Patient hat eine Stuhlprobe abgegeben und mittels 16S rRNA Sequenzierung wurde die Zusammensetzung des intestinalen Mikrobioms untersucht.
Die Mikrobiomzusammensetzung ist wie folgt: {microbiome_perc_st}.
Die Mikrobiomzusammensetzung soll bitte als Tabelle in dem Brief dargestellt werden.
Bitte berücksichtige auch diese weiteren Informationen über den Patienten in dem Bericht: {patient_information}.
Die Informationen über den Patienten sollen bitte auch einmal über dem Text, also im Briefkopf tabellarisch dargestellt werden.
Zusätzlich stelle ich dir folgendes Expertenwissen zur Verfügung: {expert_knowledge}.
Nutze dieses Expertenwissen aber bitte nur, wenn die entsprechenden Bakterien auch in der Mikrobiomzusammensetzung erwähnt werden.
Wenn ein Bakterium in der Mikrobiomzusammensetzung nicht gelistet ist, heißt das nicht, dass es nicht vorhanden ist.
Wenn es nicht gelistet ist, kann man keine Aussage über dieses Bakterium treffen.
Liste das Expertenwissen bitte nicht extra in dem Brief auf. Dieses dient nur dir als Hintergrundwissen.
Und bitte schreibe einen Absatz über die Analyse und den Vergleich zu gesunden Donoren und einen Absatz mit Handlungsempfehlungen, um eventuelle Defizite in der Mikrobiomzusammensetzung zu addressieren.
Der Brief soll außerdem direkt den Patienten addressieren und professionell klingen. Schreibe den Brief bitte auf Deutsch. Vielen Dank!
"""
...
def read_expert_knowledge():
	file = open(f'/script/expert_knowledge.txt', mode='r')
	knowledge = file.read()
	file.close()
	return knowledge

# Previous implementation of the call_LLM function
...
def call_LLM(provider: LLMProvider, template, expert_knowledge, microbiome_perc_st, patient_information):
    ...
    # Send prompt to LLM
    query_result = provider.query(formatted_prompt)
...
def create_LLM_report(result_path, pdf, width_text, a4_width_mm):
	...
	for sampleID in metadata.index:
		...
		LLM_result = call_LLM(provider, TEMPLATE, expert_knowledge, microbiome_perc_st, information)
